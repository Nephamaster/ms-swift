...
MODEL = Path('/share/project/wuhaiming/spaces/CLLM/models/Qwen3-4B-Base-Char')
DATA = Path('/share/project/wuhaiming/spaces/CLLM/data/processed/cpt/'
...
with DATA.open('r', encoding='utf-8') as f:
    for line_no, line in enumerate(f, 1):
        ...
        if 'messages' in item:
            texts = [msg['content'] for msg in item['messages'] if msg.get('role') == 'assistant']
            text = '\n'.join(texts)
        elif 'text' in item:
            text = item['text']
        else:
            raise ValueError(f'line {line_no}: unsupported keys {list(item)}')
